	...
	protected void registerModels() {
		for (String trim : TRIMS) {
			for (ArmorItem.Type type : ArmorItem.Type.values()) {
				this.existingFileHelper.trackGenerated(ResourceLocation.withDefaultNamespace("trims/items/" + type.getName() + "_trim_" + trim), PackType.CLIENT_RESOURCES, ".png", "textures");
			}
		}
		for (Item item : XOItems.ITEMS.getEntries().stream().map(RegistryObject::get).toList()) {
			switch (item) {
				case TieredItem tieredItem -> this.handheldItem(tieredItem);
	...
	private void armorItem(@NotNull ArmorItem item) {
		...
		for (int i = 0; i < TRIMS.length; i++) {
			...
			this.getBuilder(location.getPath() + "_" + TRIMS[i] + "_trim").parent(model)
				...
				.texture("layer1", ResourceLocation.withDefaultNamespace("trims/items/" + item.getType().getName() + "_trim_" + TRIMS[i]));
		}
	}
	//endregion
